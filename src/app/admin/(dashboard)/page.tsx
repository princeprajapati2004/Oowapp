...
export default async function AdminDashboardPage() {
  ...
  // getDashboardAnalytics returns a single DashboardData object (already
  // aggregated server-side) — pull today's totals straight from its summary.
  const todayStats =
    initialData.summary.totalOrders > 0
      ? { count: initialData.summary.totalOrders, revenue: initialData.summary.totalRevenue }
      : null;

  // Setup checklist — tweak the "done" conditions to match your real
  ...
  const checklist = [
    ...
    },
    {
      ...
      href: "/admin/settings",
      done: Boolean(shop.businessName),
    },
  ];
            ...
            {checklist.map((item) => (
              ...
              >
                <span className="flex items-center gap-2.5 text-sm">
                  ...
                  <span className={cn("leading-tight", item.done && "text-muted-foreground line-through")}>
                    {item.label}
                  </span>
                </span>
