...
import { getItemReportData, type ItemReportFilters } from "@/lib/services/reports/item-report";
...
export async function GET(request: Request) {
  try {
    ...
    const filters: ItemReportFilters = {
      from: range.from,
      to: range.to,
    ...
    const { summary, rows, total, truncated } = await getItemReportData(
      session.shopId,
      filters,
