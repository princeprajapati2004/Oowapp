...
import { getDiscountReportSummary, listDiscountReportRows, type DiscountReportFilters } from "@/lib/services/reports/discount-report";
...
export async function GET(request: Request) {
  try {
    ...
    const [summary, { rows, total, truncated }] = await Promise.all([
      getDiscountReportSummary(session.shopId, filters),
      listDiscountReportRows(session.shopId, filters, all ? { all: true } : { page, pageSize }),
    ]);
