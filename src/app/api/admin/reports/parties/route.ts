...
import { getPartyReportData, type PartyReportFilters } from "@/lib/services/reports/party-report";
...
export async function GET(request: Request) {
  try {
    ...
    const filters: PartyReportFilters = {
      from: range.from,
      to: range.to,
      search: searchParams.get("search") || undefined,
      type: typeParam === "CUSTOMER" || typeParam === "SUPPLIER" ? typeParam : undefined,
    };
    ...
    const { summary, rows, total, truncated } = await getPartyReportData(
      session.shopId,
      filters,
